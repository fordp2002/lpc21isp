...
static int CheckCommand(ISP_ENVIRONMENT *IspEnvironment, const char *FormattedCommand)
{
	FormatCommand(Answer, Answer);

	return strcmp(Answer, FormattedCommand);
}
...
#define local_static
	...
	if (IspEnvironment->Blind == 0)
	{
		ReceiveComPort(IspEnvironment, Answer, sizeof(Answer) - 1, &realsize, 2, 1000);
		if (CheckCommand(IspEnvironment, "Synchronized\nOK\n") != 0)
		{
	...
	if (IspEnvironment->Blind == 0)
	{
		...
		FormatCommand(Answer, Answer);
		if ((IspEnvironment->Blind == 0) && (strncmp(Answer, temp, strlen(temp)) != 0))
		{
	...
	*/
	...
	while (1)
	{
		...
		for (SectorOffset = 0; SectorOffset < SectorLength; SectorOffset += SectorChunk)
		{
			...
			// Write multiple of 45 * 4 Byte blocks to RAM, but copy maximum of one sector to Flash
				...
				LPCtypes[IspEnvironment->DetectedDevice].ChipVariant == CHIP_VARIANT_LPC43XX)
			{
				...
				for (Pos = SectorStart + SectorOffset; (Pos < SectorStart + SectorOffset + CopyLength) && (Pos < IspEnvironment->BinaryLength); Pos += (45 * 4))
				{
					for (Block = 0; Block < 4; Block++)  // Each block 45 bytes
					{
						...
						FormatCommand(Answer, Answer);
						if ((IspEnvironment->Blind == 0) && (strncmp(Answer, tmpString, tmpStringPos) != 0))
						{
							DebugPrintf(1, "Error on writing data (1)\n");
							return (ERROR_WRITE_DATA);
						}
		...
		if (IspEnvironment->Blind == 0)
		{
			...
			{ // Skip response on G command - show response on Terminal instead
				...
				*/
				...
				FormatCommand(Answer, Answer);
				if ((IspEnvironment->Blind == 0) && (realsize == 0 || strncmp((const char *)Answer, /*cmdstr*/ExpectedAnswer, strlen(/*cmdstr*/ExpectedAnswer)) != 0))
				{
