...
static void OpenSerialPort(ISP_ENVIRONMENT *IspEnvironment)
{
	...
	dcb.ByteSize    = 8;
	dcb.StopBits    = (IspEnvironment->Blind) ? TWOSTOPBITS : ONESTOPBIT;			//Two stop bit for blind programming to allow simple forwarding without buffering
	dcb.Parity      = NOPARITY;
...
#if 1
	if (IspEnvironment->Blind)
	{
		unsigned long Timeout = (s[0] == 'E') ? 150 : 40;
						  ...
						  unsigned timeOutMilliseconds)
{
	...
	*RealSize = 0;
	...
	sprintf(tmp_string, "\nAnswer %ld (Length=%ld): ", GetTickCount() - StartCount, (*RealSize));

	{
		DWORD TotalTime = GetTickCount() - StartCount;
		DebugPrintf(2, "%ld:", TotalTime);

		if (TotalTime > 48)
		{
			DumpString(2, Answer, (*RealSize), tmp_string);
			return;
		}
	}
...
static void ReadArguments(ISP_ENVIRONMENT *IspEnvironment, unsigned int argc, char *argv[])
{
	...
	if (argc >= 5)
	{
		for (i = 1; i < argc - 3; i++)
		{
			...
			if (strnicmp(argv[i], "-blind", 6) == 0)
			{
				char* num;
				num = argv[i] + 6;
				IspEnvironment->Blind =  atoi(num);
				DebugPrintf(3, "Blind programming of chip %08lX enabled.\n", IspEnvironment->Blind);
				continue;
			}
	...
	if (argc < 5)
	{
		...
		DebugPrintf(1, "Syntax:  lpc21isp [Options] file[ file[ ...]] comport baudrate Oscillator_in_kHz\n\n"
			...
			"         -blind       Program without the need to check or have a recieve line\n"
...
int PerformActions(ISP_ENVIRONMENT *IspEnvironment)
{
	...
	if ((IspEnvironment->ProgramChip) && (IspEnvironment->DetectOnly == 0))
	{
...
#endif
{
	...
	Result = PerformActions(&IspEnvironment);                   // Do as requested !
